# publish_eye_in_hand_calibration.launch.py
# Publishes a saved eye-in-hand calibration as a static TF from fr3_link8
# to wrist_camera_link. The saved transform ends at the optical frame; this
...
def generate_launch_description():
    ...
    camera_root_arg = DeclareLaunchArgument(
        'camera_root_frame',
        default_value='wrist_camera_link',
        description='Camera root frame to attach to the robot effector. The '
                    'camera driver must already publish a static TF from the '
                    'tracking base frame (wrist_camera_color_optical_frame) '
                    'to this frame.',
    )
